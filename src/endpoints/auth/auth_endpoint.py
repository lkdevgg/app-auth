...
from auth.userSerializers import userEntity, userResponseEntity
from auth import user_schema, user_utils
...
async def create_user(payload: user_schema.CreateUserSchema):
...
@router.post('/login')
def login(payload: user_schema.LoginUserSchema, response: Response, Authorize: AuthJWT = Depends()):
